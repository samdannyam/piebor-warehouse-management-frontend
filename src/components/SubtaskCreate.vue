<template>
  <v-card class="px-5 bg-main rounded-xl">
    <v-card-title class="pt-4 pl-1 text-core"> Create Subtask </v-card-title>
    <v-row>
      <v-col cols="6">
        <v-text-field
    ...
    async saveNewSubTask() {
      try {
        const taskInfo = {
          ...
          assignedBy: localStorage.getItem("user_Id"), //CHANGE
        };
        const query = {
          taskInfo,
          parentTaskId: this.parentId,
        };
        const response = await this.axios.post("/v1/task/createSubTask", query);
        if (response.status === 201) {
          (this.newTask = {}), (this.createSubTaskDialog = false);
          this.success = true;
          this.filter();
        }
      } catch (error) {
